...
module.exports = function(sails) {
    ...
    async.auto({
      ...
      registry: ['hooks',
        function populateRegistry(cb) {
          ...
          console.log("populateRegistry() cb()");
          cb();
        }
      ],
  ...
  function loadHooks(cb) {
    ...
    console.log("Series for loadHooks.");
    async.series([

      function(cb) {
        console.log("loadHookDefinitions.");
        loadHookDefinitions(sails.hooks, cb);
      },
      function(cb) {
        console.log("initializeHooks.");
  ...
  function loadHookDefinitions(hooks, cb) {
    console.log("Loading loadHookDefinitions.");
   ...
   */
  function ready__(cb) {
    console.log("Loading ready__.");
    return function(err) {
      if (err) {
        console.log("Err and callback in ready.", err);
        return cb && cb(err);
      }
      ...
      console.log("Returning callback, sails object.");
      cb && cb(null, sails);
    };
  }
};
