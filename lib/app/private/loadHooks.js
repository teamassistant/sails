...
module.exports = function(sails) {
  ...
  return function initializeHooks(hooks, cb) {
    ...
    function loadHook(id, cb) {
      ...
      console.log(`id=${id}`);
      console.log(`sails.config[hooks[id].configKey=${sails.config[hooks[id].configKey || id]}`);
      console.log(`sails.config[hooks[id].configKey || id]._hookTimeout=${sails.config[hooks[id].configKey || id] ? sails.config[hooks[id].configKey || id]._hookTimeout : null}`);
      console.log(`Hook timeout sails.config.hookTimeout=${sails.config.hookTimeout}`);
      // console.log(`Hook timeout sails.dynamicConfig.hookTimeout=${sails.dynamicConfig.hookTimeout}`);
      var hookTimeout;
      if (id != 'userhooks') {
